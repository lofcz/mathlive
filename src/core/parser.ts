...
export class Parser {
  ...
  /** Macros registered during this parse (e.g. via `\DeclareMathOperator`). */
  private localMacros: Record<string, MacroDefinition> = {};
  ...
  /**
   * Register a macro for the remainder of this parse
   * (used by `\DeclareMathOperator`, etc.).
   */
  defineLocalMacro(name: string, def: MacroDefinition): void {
    const key = name.startsWith('\\') ? name.slice(1) : name;
    if (!key) return;
    this.localMacros[key] = def;
  }
